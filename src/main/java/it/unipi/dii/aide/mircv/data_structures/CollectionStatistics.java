...
public final class CollectionStatistics {
    ...
    public static void readCollectionStatsFromDisk() {
        ...
        try (
            ...
            ByteBuffer statsBuffer = ByteBuffer.allocate(Integer.BYTES + Double.BYTES);   // bytes to read from disk
    ...
    public static void storeCollectionStatsIntoDisk() {
        ...
        try (
            ...
            MappedByteBuffer buffer = channel.map(FileChannel.MapMode.READ_WRITE, 0, Integer.BYTES + Double.BYTES); // integer size * number of int to store (1) + double size * number of double to store (1)

...
public final class Query {
    ...
    public static void executeQuery(String q, int k) throws IOException {
    ...
    public static void executeQueryPQ(String q, int k) throws IOException {
        long startTime = System.currentTimeMillis();
        query = TextProcessor.preprocessText(q);
        Query.k = k;
        pq_res = new PriorityQueue<>(k, new CompareRes());
...
//        ArrayList<PostingList> postingLists = new ArrayList<>();
        ...
        try (
            ...
            pq_DAAT = new PriorityQueue<>(query.size(), new CompareScore());
                ...
                if(firstIter) {
                    ...
                    if (pb.getDocId() == acc.getDocId()) {
                        ...
                        if (pq_res.size() == k) {
                            ...
                            if (acc.getScore() > pq_res.peek().score) {
                                pq_res.poll();
     ...
     */
    private static class CompareScore implements Comparator<DAATBlock> {
        @Override
